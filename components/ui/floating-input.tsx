...
export const FloatingInput = React.forwardRef<
  HTMLInputElement,
  FloatingInputProps
>(({ label, leftItem, rightItem, className, type = "text", ...props }, ref) => {
      ...
      {label && (
        <label
          className={cn(
            ...
            "backdrop-blur-xl bg-white/70 dark:bg-neutral-800/60 text-muted-foreground",
            leftItem ? "left-10" : "left-3",
            focused || props.value
              ? "text-xs -top-2 text-primary font-medium"
              : "text-sm top-3.5",
